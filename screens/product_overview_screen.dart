...
class _ProductOverviewScreenState extends State<ProductOverviewScreen> {
  ...
  @override
  void initState() {
    // Provider.of<Products_Provider>(context).fetchProducts(); ---> // this don't work here becouse iniState don't have a correct wire up for context
    // so to solve this we can use

    // Provider.of<Products_Provider>(context, listen: false).fetchProducts();   --> listen:false also work

    // Future.delayed(Duration.zero).then(
    //   (value) => Provider.of<Products_Provider>(context, listen: false)
    //       .fetchProducts(),
    // );
  ...
  void didChangeDependencies() {
    if (isInit) {
      ...
      Provider.of<Products_Provider>(context).fetchProducts().then(
            (value) => setState(() {
              isLoading = false;
            }),
          );
